...
func TestFileWriter_WithBoxReplacement(t *testing.T) {
	...
	_, _ = inputFile.Write(buildFtypBox())
	...
	_, _ = inputFile.Write(make([]byte, 8)) // size + type

	// meta version+flags
	_, _ = inputFile.Write([]byte{0x00, 0x00, 0x00, 0x00})

	// idat child
	idatData := []byte("original item data")
	idatBox := buildGenericBox("idat", idatData)
	_, _ = inputFile.Write(idatBox)

	// Go back and write meta size
	metaEnd, _ := inputFile.Seek(0, io.SeekCurrent)
	metaSize := uint32(metaEnd - metaStart)
	_, _ = inputFile.Seek(metaHeaderPos, io.SeekStart)
	_ = binary.Write(inputFile, binary.BigEndian, metaSize)
	_, _ = inputFile.Write([]byte("meta"))
	_, _ = inputFile.Seek(metaEnd, io.SeekStart)

	// mdat
	_, _ = inputFile.Write(buildMdatBox([]byte("pixel data")))
...
func TestFileWriter_MdatPreservation(t *testing.T) {
	...
	_, _ = inputFile.Write(buildFtypBox())
	_, _ = inputFile.Write(buildMdatBox(pixelData))
	_ = inputFile.Close()
	...
	modifier := NewBoxTreeModifier()
	_ = WriteModifiedFile(inputFile.Name(), outputPath, tree, modifier)
	...
	_, _ = outFile.Seek(int64(payloadOffset), io.SeekStart)

	mdatPayload := make([]byte, mdatNode.Size-mdatNode.HeaderSize)
	_, _ = io.ReadFull(outFile, mdatPayload)
...
func TestWriteBoxHeaderWithSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, _ = io.ReadFull(tmpFile, header)
			_ = tmpFile.Close()
